    ...
    <div>
        ...
        <el-row style="width:80%;margin-left:10%;">
            <el-col :span="18" style="width:65%;margin-top:1.36%;margin-left:1.4%;text-align:left">
                <div class="searchDiv">
                    <el-input class="searchInput" placeholder="Search items here" prefix-icon="iconfont el-icon-search" v-model="search_input"></el-input>
                </div>                        
            </el-col>
...
<script>
    ...
    export default {
        ...
        mounted() {
            ...
            const { Conflux, util, Contract } = require('js-conflux-sdk');
            ...
            //^mark 这里的account应该是 连接钱包的account
            //
        ...
        methods: {
            async fillNFTInfoBrief(nftIds, target){
                let _this = this;
                const { Conflux, util, Contract } = require('js-conflux-sdk');
                ...
                for(var i = 0; i < nftIds.length; i++)
                {
                    if(nftIds[i].length > 0) {
                        ...
                        if(nftIds[i][0] == undefined) continue;
                        // console.log(userOwnedNFT[i]);
                        let itemId = nftIds[i][0];
                        let itemInfo = await contract.getItemInfo(itemId);
                        let tokenId = itemInfo[2][0];
