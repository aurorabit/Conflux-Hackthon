<template>
    <div>
            ...
            <el-col :span="12" style="margin-top:2.1%">
                ...
                <el-row type="flex" style="text-align:left;">
                    <el-col :span="8" style="margin-left:3%;margin-top:50px">
                        <el-row style="font-size:50px;">{{ nft_info.sell_price }} CFX </el-row>
                        <el-row style="text-align:left;color:grey;font-weight: bold;margin-top:-10px">&ensp;Sale Price</el-row>
                        <el-row style="margin-top:10px"><el-button class="buyBtn" size="medium" @click="dialogVisible1 = true"><div class="buy_font">BUY</div></el-button></el-row>
                    </el-col>
                    <el-col :span="6" :offset="2" style="margin-left:3%;margin-top:50px">
                        <el-row style="font-size:50px">{{ nft_info.loan_price }} CFX </el-row>
                        <el-row style="text-align:left;color:grey;font-weight: bold;margin-top:-10px">&ensp;Loan Price</el-row>
                        <el-row style="margin-top:10px"><el-button class="loanBtn"  size="medium" @click="dialogVisible2 = true"><div class="buy_font">LOAN</div></el-button></el-row>
                    </el-col>
                </el-row>
            </el-col>
        ...
        <span slot="footer" class="dialog-footer" style="vertical-align:middle" >
            <el-row  style="text-align:center;vertical-align:middle">
                <el-col :span="12" style="text-align:center">
                    <button class="checkoutBtn" @click="buyConfirm" style="margin-left:30%"><div class="checkoutFont">Checkout</div></button>
                </el-col>
                <el-col :span="12" style="text-align:center">
                    <button class="cancelBtn" @click="dialogVisible1 = false"  style="margin-right:30%"><div class="cancelFont">Cancel</div></button>
                </el-col>
            </el-row>
        </span>
        </el-dialog>
        ...
        <span slot="footer" class="dialog-footer" style="vertical-align:middle" >
            <el-row  style="text-align:center;vertical-align:middle">
                <el-col :span="12" style="text-align:center">
                    <button class="checkoutBtn" @click="loanConfirm" style="margin-left:30%"><div class="checkoutFont">Checkout</div></button>
                </el-col>
                <el-col :span="12" style="text-align:center">
                    <button class="cancelBtn" @click="dialogVisible2 = false"  style="margin-right:30%"><div class="cancelFont">Cancel</div></button>
                </el-col>
            </el-row>
        </span>
        </el-dialog>
    ...
    export default {
        ...
        methods: {
            ...
            async getJsonUrl(tokenId) {
                ...
                let _this = this;
                const contract = cfx.Contract({ abi, address:MathArtAddress,bytecode:bytecode});
            ...
            buyConfirm() {
                let _this = this;
                const {Conflux, util, Drip} = require('js-conflux-sdk');
                ...
                async function main() {
                    ...
                    // 具体可以参考智能合约 17到30行
                    // https://github.com/aurorabit/NFiT-core/blob/main/contracts/NFiTMarket.sol
                    console.log("具体的内容");
                    console.log("第一项是itemId");
                    console.log(itemInfo[0][0]);
                    console.log("第二项是nftContract");
                    console.log(itemInfo[1]);
                    console.log("第三项是tokenId");
                    console.log(itemInfo[2][0]);
                    ...
                    console.log(itemInfo[6]);
                    console.log('第八项是loanPrice')
                    console.log(itemInfo[7])
                    console.log('第九项是redeemPrice')
                    console.log(itemInfo[8])
                    console.log('第十项是deadline')
                    console.log(itemInfo[9])
                    console.log('第十一项是state')
                    console.log(itemInfo[10])
                    }
                    let price = _this.nft_info.sell_price;
                    // function buyNFT (uint itemId) public payable {
                    let transaction = await contract
                        .buyNFT(itemId)
                        .sendTransaction({
                        from: _this.account,
                        to: NFiTMarketAddress,
                    ...
                    main().catch(e => {
                        console.error(e);
                        _this.$message.error("Fail to buy NFT!");
                    });
                },
                loanConfirm() {
                    let _this = this;
                    ...
                    async function main() {
                        ...
                        console.log(_this.account);
                        let itemId = _this.nft_id;
                        let loanPrice = _this.nft_info.loan_price;
                        let transaction = await contract
                        .receiveNFT(itemId)
                        .sendTransaction({
                        from: _this.account,
                        to: NFiTMarketAddress,
                        value: Drip.fromCFX(loanPrice),
                        gas: 300000
                        }).executed();
                        // alert("you loan a nft");
                        _this.dialogVisible2 = false;
                        _this.$message.success("Successfully loan NFT!");
                        _this.$router.push({path:'/mine',query: {name: '1'}});
                    }
                    main().catch(e=>{
                        console.error(e);
                        _this.$message.error("Fail to loan NFT");
                    })
                    return ;
            },
            async getNFTInfo(itemUrl){
                ...
                async function main() {
                    ...
                    if(itemInfo[6].length > 0)
                        _this.nft_info.sell_price = new Drip(itemInfo[6].toString()).toCFX();
                    else 
                        _this.nft_info.sell_price = 0;
                    
                    if(itemInfo[7].length > 0)
                        _this.nft_info.loan_price = new Drip(itemInfo[7].toString()).toCFX();
                    else
                        _this.nft_info.loan_price = 0;

                    _this.nft_info.royality = itemInfo[9][0];
                }
                main().catch(e => {
                    console.error(e);
